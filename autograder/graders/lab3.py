...
def grade_solution(student_output, solution):
  ...
  for field in solution:
    ...
    if solution[field]['match'] == 'exact':
      ...
      if not grades[field]['match']:
        grades[field]['expected'] = '\n'.join(list(expected_lines.difference(obtained_lines)))
        grades[field]['obtained'] = '\n'.join(list(obtained_lines.difference(expected_lines)))
...
def log_results(log_file, report, verbose=False):
  ...
  for subtask in sorted(report['evaluation_results'], reverse=False):  # Only one task currently: ID3
    log_file.write(f"\n== {subtask.upper()} ==\n")
    ...
    passed_fields = {'BRANCHES': 0, 'PREDICTIONS': 0, 'ACCURACY': 0, 'CONFUSION_MATRIX': 0}
    for i in test_instances:
      if not i['test_passed'] and (not i['execute'] or not i['timeout']):
        continue
      for field in i['field_results']:
        if i['field_results'][field]['match']:
          passed_fields[field] += 1
    total_instances = len(test_instances)
    for field in passed_fields:
      log_file.write(f"    {field}: {passed_fields[field]} / {total_instances}\n")
